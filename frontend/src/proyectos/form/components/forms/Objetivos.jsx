...
// Importamos el API centralizado de planificación
...
export default function Objetivos({ data, updateData }) {
  ...
  // 1. Cargar los ODS utilizando catalogoPlanificacionApi
  useEffect(() => {
    const fetchOds = async () => {
      try {
        const response = await catalogoPlanificacionApi.obtenerODS();
        const resultados = response.results ? response.results : response;
        const odsOrdenados = resultados.sort((a, b) => a.numero - b.numero);
