...
export default function Cronograma({ data, updateData }) {
  ...
  const obtenerErrorFila = (item) => {
    if (!item.descripcion?.trim() || !item.fecha_inicio || !item.fecha_fin) return "Faltan completar campos obligatorios.";
    if (item.fecha_fin < item.fecha_inicio) return "La fecha de fin no puede ser anterior a la de inicio.";
    ...
    if (fueraRangoGeneral) return "Fechas fuera del rango general del proyecto.";
    return null;
  };
              ...
              return (
                <tr key={index} className={`transition-colors ${errorFila ? 'bg-red-50/10' : 'hover:bg-slate-50/50'}`}>
                  ...
                  <td className="p-2">
                    ...
                    />
                    {inicioFueraRango && <span className="text-[10px] text-red-600 block mt-0.5 font-medium">⚠️ Fuera de rango</span>}
                  </td>
                  <td className="p-2">
                    <input
                      type="date"
                    ...
                    />
                    {errorLocal && <span className="text-[10px] text-red-600 block mt-0.5 font-medium">⚠️ Fin menor a Inicio</span>}
                    {finFueraRango && !errorLocal && <span className="text-[10px] text-red-600 block mt-0.5 font-medium">⚠️ Fuera de rango</span>}
                  </td>
                  <td className="p-2">
                    <input
                      type="text"
