...
import { 
  FiSearch, FiBook, FiFileText, FiEye, FiX, 
  FiCalendar, FiUser, FiMapPin, FiDollarSign, 
  FiClock, FiCheckCircle, FiLoader, FiChevronLeft, FiChevronRight
} from "react-icons/fi";
import { useRepositorio } from "./hooks/useRepositorio"; // <--- Hook separado
  ...
  const renderEstadoBadge = (estado) => {
    ...
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-50 text-slate-700 border border-slate-200">
        {estado?.toUpperCase().replace(/_/g, ' ')}
      </span>
    );
  };
              ...
              {proyectosRepositorio.map((proyecto) => (
                <div key={proyecto.id} className={viewMode === 'grid' ? "bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col justify-between shadow-xs hover:shadow-md transition-shadow" : "bg-white rounded-xl border border-slate-200 p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xs"}>
                  ...
                  <div className={viewMode === 'grid' ? "p-4 pt-0 grid grid-cols-2 gap-2" : "flex items-center gap-2 shrink-0 w-full sm:w-auto"}>
                    ...
                    {/* <button 
                      disabled={proyecto.estado !== 'aprobado' && proyecto.estado !== 'en_ejecucion'} 
                      className={`flex items-center justify-center gap-1.5 h-9 rounded-lg text-xs font-semibold w-full sm:px-4 ${proyecto.estado === 'aprobado' || proyecto.estado === 'en_ejecucion' ? 'bg-slate-100 text-[#b1122b] hover:bg-red-50' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
                    >
                      <FiFileText /> Informe
                    </button>*/}
                  </div>
            ...
            <div className="p-6 md:p-8 space-y-8 flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-slate-700 truncate max-w-[140px] block">{proyectoSeleccionado.lugar_ejecucion || 'No especificado'}</span>
                  </div>
                </div>
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-slate-700">S/. {proyectoSeleccionado.monto_financiamiento || '0.00'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <ul className="text-xs text-slate-600 space-y-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    ...
                    <li><b>Participantes totales:</b> {proyectoSeleccionado.nro_docentes || 0} Docentes y {proyectoSeleccionado.nro_estudiantes || 0} Estudiantes</li>
                  </ul>
                </div>
                <div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px] text-slate-500">
                  <div><b>Creado:</b> {proyectoSeleccionado.created_at?.substring(0,10) || 'N/A'}</div>
                </div>
              </div>
