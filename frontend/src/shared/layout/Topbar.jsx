...
import { notificacionApi } from "../api/proyectos/notificacionApi";
...
export default function Topbar() {
  ...
  // ── Notificaciones (antes useNotificaciones.js, sin carpeta definida -> inline aquí) ──
  const [notificaciones, setNotificaciones] = useState([]);

  const fetchNotificaciones = useCallback(async () => {
    try {
      const data = await notificacionApi.obtenerNotificaciones();
  ...
  const [userData, setUserData] = useState({
    ...
    role: localStorage.getItem("user_role") || "Sin rol",
    ...
    const sincronizarUsuario = async () => {
      try {
        ...
        const actualizado = {
          ...
          role: data.rol_nombre || localStorage.getItem("user_role") || "Sin rol",
        ...
        setUserData((prev) => ({ ...prev, initial: prev.name.charAt(0).toUpperCase() }));
      }
    };
    ...
    <header className="h-[72px] bg-white border-b border-slate-200 flex items-center justify-between px-6 lg:px-8 sticky top-0 z-20 w-full">
      ...
      <div className="flex items-center gap-5 ml-auto pl-4">
            ...
            <div className="absolute right-0 mt-3 w-80 max-h-96 overflow-y-auto bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 animate-in fade-in slide-in-from-top-2">
              <div className="px-4 py-2 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white gap-2">
                ...
                <div className="flex items-center gap-2">
                  {unreadCount > 0 && (
                    <button onClick={marcarTodasComoLeidas} className="flex items-center gap-1 text-[11px] font-semibold text-[#b1122b] hover:text-[#8a0e21] transition-colors">
              ...
              {notificaciones.length === 0 ? (
                <div className="p-4 text-center text-slate-500 text-sm">No tienes notificaciones</div>
              ) : (
              ...
              <div className="px-4 pt-2 pb-1 border-t border-slate-100 sticky bottom-0 bg-white">
                <Link to="/notificaciones" onClick={() => setIsNotifOpen(false)} className="block text-center text-xs font-semibold text-[#b1122b] hover:text-[#8a0e21] py-1.5 transition-colors">
        ...
        <div className="relative" ref={dropdownRef}>
          <div className="flex items-center gap-3 cursor-pointer p-1 -m-1 rounded-lg hover:bg-slate-50 transition-colors" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            ...
            <div className="hidden md:flex flex-col">
              <strong className="text-sm font-medium text-slate-800 leading-tight capitalize">{userData.name.toLowerCase()}</strong>
              <span className="text-[11px] text-slate-500 tracking-wide uppercase mt-0.5">{userData.role}</span>
            </div>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 mt-3 w-[260px] bg-white rounded-2xl shadow-xl border border-slate-200 py-3 origin-top-right animate-in fade-in slide-in-from-top-2">
              <div className="px-5 pb-3">
                <strong className="block text-[15px] font-medium text-slate-800 capitalize leading-snug">{userData.name.toLowerCase()}</strong>
                <span className="block text-[13px] text-slate-500 mb-3 truncate">{userData.email}</span>
                <span className="inline-block bg-blue-100 text-blue-700 text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wide">{userData.role}</span>
              </div>
              ...
              <button onClick={handleLogout} className="w-full text-left px-5 py-2.5 text-[15px] text-red-600 hover:bg-red-50 flex items-center gap-3 transition-colors">
